...
static char *extract_filename_in_comments(const char *query_string);
				 ...
				 ParamListInfo boundParams)
{
    ...
    JSON_Value *json_rel = relOptInfoToJson(final_rel, root);

    JSON_Obj *root_object = json_value_get_object(json_rel);

    char *file_name = extract_filename_in_comments(query_string);
    char file_path[256];

    if(file_name == NULL){
        sprintf(file_path, "%s", "/tmp/temp.json");
    }else{
        sprintf(file_path, "/tmp/%s.json", file_name);
        free(file_name);
    }
    json_serialize_to_file_pretty(json_rel, file_path); 

    json_value_free(json_rel);
...
char *extract_filename_in_comments(const char *query_string){
    const char *start;
    const char *end;
    size_t comment_length = 0;

    if((start = strstr(query_string, "/*")) != NULL){
        end = strstr(query_string, "*/");

        start += 2;
        comment_length = end - start;

        char* comment = (char *)malloc(comment_length + 1);
        strncpy(comment, start, comment_length);
...
JSON_Value *relOptInfoToJson(RelOptInfo *rel, PlannerInfo *root)
{
    JSON_Value *root_value = json_value_init_object();
    JSON_Obj *root_object = json_value_get_object(root_value);

    JSON_Value *pathlist_value = json_value_init_array();
    JSON_Array *pathlist_array = json_value_get_array(pathlist_value);

    ListCell *lc;
    foreach(lc, rel->pathlist)
    {
        Path *path = (Path *) lfirst(lc);
        JSON_Value *path_value = pathToJson(path, root);
        json_array_append_value(pathlist_array, path_value);
    }

    json_obj_set_value(root_object, "pathlist", pathlist_value);

    return root_value;
}

JSON_Value *pathToJson(Path *path, PlannerInfo *root)
{
    if(path->pathtype == T_Result){
        ProjectionPath *projection_path = (ProjectionPath *) path;
        return pathToJson(projection_path->subpath, root);
    }

    JSON_Value *path_value = json_value_init_object();
    JSON_Obj *path_object = json_value_get_object(path_value);

    char *tag_str = nodeTagToString(path->pathtype);

    if(strcmp(tag_str, "Unknown") == 0){
        json_obj_set_number(path_object, "pathtype", path->pathtype);
    }else{
        json_obj_set_string(path_object, "pathtype", nodeTagToString(path->pathtype));
    }

    json_obj_set_number(path_object, "startup_cost", path->startup_cost);
    json_obj_set_number(path_object, "total_cost", path->total_cost);
    ...
    switch (path->pathtype) {
        ...
        case T_HashJoin: {
            ...
            json_obj_set_value(path_object, "outer_join_path",
                                  pathToJson(join_path->outerjoinpath, root));
            json_obj_set_value(path_object, "inner_join_path",
                                  pathToJson(join_path->innerjoinpath, root));
            break;
        }
        case T_SeqScan:
        case T_SampleScan:
        ...
        case T_ForeignScan:
        case T_CustomScan: {
//            if(path->parent->baserestrictinfo != NULL){
//
//                ListCell *lc;
//                foreach(lc, path->parent->baserestrictinfo){
//                    RestrictInfo *restrict_info = lfirst(lc);
//
//                    if(restrict_info -> clause){
//                        FILE * file;
//
//                        if((file = fopen("/tmp/restrict.txt", "a")) == NULL){
//                            perror("fopen");
//                            exit(1);
//                        }
//                        fprintf(file, "%s\n", nodeToString(restrict_info->clause));
//                    }
//                }
//            }

            RangeTblEntry *rte = root->simple_rte_array[path->parent->relid];
            char *relname = get_rel_name(rte->relid);
            json_obj_set_string(path_object, "relname", relname);

            if(path->parent->baserestrictinfo != NULL) {

                JSON_Value *filter_value = json_value_init_object();
                JSON_Obj *filter_object = json_value_get_object(filter_value);

                json_obj_set_string(filter_object, "pathtype", "FILTER");
                json_obj_set_number(filter_object, "total_cost", path->total_cost);
                json_obj_set_value(filter_object, "path", path_value);
        ...
        case T_IndexPath:{
            ...
            json_obj_set_number(path_object, "index_cost", index_path->indextotalcost);
            break;
        }
        case T_BitmapHeapPath:{
            BitmapHeapPath *bitmap_heap_path = (BitmapHeapPath *)path;
            json_obj_set_value(path_object, "path", pathToJson(bitmap_heap_path->bitmapqual, root));
            break;
        }
        case T_BitmapAndPath:{
            BitmapAndPath *bitmap_and_path = (BitmapAndPath *)path;
            ListCell *lc;

            JSON_Value * pathlist_value = json_value_init_array();
            JSON_Array *pathlist_array = json_value_get_array(pathlist_value);

            foreach(lc, bitmap_and_path->bitmapquals){
                Path * next_path = (Path*)lfirst(lc);
                json_array_append_value(pathlist_array, pathToJson(next_path, root));
            }
            json_obj_set_value(path_object, "pathlist", pathlist_value);
            break;
        }
        case T_BitmapOrPath:{
            BitmapOrPath *bitmap_or_path = (BitmapOrPath *)path;
            ListCell *lc;

            JSON_Value * pathlist_value = json_value_init_array();
            JSON_Array *pathlist_array = json_value_get_array(pathlist_value);

            foreach(lc, bitmap_or_path->bitmapquals){
                Path * next_path = (Path*)lfirst(lc);
                json_array_append_value(pathlist_array, pathToJson(next_path, root));
            }
            json_obj_set_value(path_object, "pathlist", pathlist_value);
        ...
        case T_SubqueryScanPath:{
            SubqueryScanPath * subquery_scan_path = (SubqueryScanPath *)path;
            json_obj_set_value(path_object, "path", pathToJson(subquery_scan_path->subpath, root));
            break;
        }
        case T_Append:
        case T_MergeAppend:
        ...
        case T_Material:{
            ...
            json_obj_set_value(path_object, "path", pathToJson(material_path->subpath, root));
            break;
        }
        case T_Memoize:{
            MemoizePath *memoize_path = (MemoizePath *)path;
            json_obj_set_value(path_object, "path", pathToJson(memoize_path->subpath, root));
            break;
        }
        case T_Unique:{
            UniquePath *unique_path = (UniquePath *)path;
            json_obj_set_value(path_object, "path", pathToJson(unique_path->subpath, root));
            break;
        }
//        case T_Result: {
//            ProjectionPath *projection_path = (ProjectionPath *) path;
//            json_obj_set_value(path_object, "path", pathToJson(projection_path->subpath, root));
//            break;
//        }
        case T_Sort:
        case T_IncrementalSort: {
            SortPath *sort_path = (SortPath *) path;
            json_obj_set_value(path_object, "path", pathToJson(sort_path->subpath, root));
            break;
        }
        case T_Group:{
            GroupPath *group_path = (GroupPath *)path;
            json_obj_set_value(path_object, "path", pathToJson(group_path->subpath, root));
            break;
        }
        case T_Agg:{
            AggPath *agg_path = (AggPath *)path;
            json_obj_set_value(path_object, "path", pathToJson(agg_path->subpath, root));
            break;
        }
        case T_WindowAgg:{
            WindowAggPath *window_agg_path = (WindowAggPath *)path;
            json_obj_set_value(path_object, "path", pathToJson(window_agg_path->subpath, root));
            break;
        }
        case T_SetOp:{
            SetOpPath *set_op_path = (SetOpPath *)path;
            json_obj_set_value(path_object, "path", pathToJson(set_op_path->subpath, root));
            break;
        }
        ...
        case T_Limit: {
            ...
            json_obj_set_value(path_object, "path", pathToJson(limit_path->subpath, root));
            break;
        }
        default:
            break;
